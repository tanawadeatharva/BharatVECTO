...
The powertrain can be configured for several different situations depending on which retarder and which gearbox is used. 
Also the output values in the [Modfile](#modal-results-.vmod) depict different points in the powertrain depending on the current configuration. 
Here are some overview which show some of the values and which position in the powertrain they represent:
